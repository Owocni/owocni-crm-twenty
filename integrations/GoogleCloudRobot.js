...
const GA4_DEBUG_MODE = process.env.GA4_DEBUG_MODE === 'true' || process.env.GA4_DEBUG_MODE === '1';
const GA4_DEBUG_VALIDATE = process.env.GA4_DEBUG_VALIDATE === 'true' || process.env.GA4_DEBUG_VALIDATE === '1';
...
const GOOGLE_ADS_CUSTOMER_ID = (process.env.GOOGLE_ADS_CUSTOMER_ID || '8037312085').replace(/-/g, '');
const GOOGLE_ADS_LEAD_CONVERSION_ACTION_ID = process.env.GOOGLE_ADS_LEAD_CONVERSION_ACTION_ID || '6897478455';
const GOOGLE_ADS_PURCHASE_CONVERSION_ACTION_ID = process.env.GOOGLE_ADS_PURCHASE_CONVERSION_ACTION_ID || '7512561962';
...
const GOOGLE_ADS_API_VERSION = (process.env.GOOGLE_ADS_API_VERSION || '22').replace(/^v/i, '');
...
else if (GA4_DEBUG_VALIDATE) console.log('🔧 GA4: DEBUG_VALIDATE (tylko walidacja)');
...
if (deletedCount > 0) {
console.log(`🧹 Cleaned up ${deletedCount} old "done" tasks from collection`);
}

    // KROK 1: Pobierz Pricing Config z Google Sheets (raz na request)
    const pricingConfig = await getPricingConfigFromSheets();
    ...
    const doneTasks = statusCheck.filter(t => t.status === 'done' || t.status === 'processed_ga4');
    if (doneTasks.length > 0) {
      console.error(`❌ CRITICAL: Found ${doneTasks.length} tasks with status "done" or "processed_ga4" in the processing queue!`);
      ...
      console.log(`✅ After filtering: ${filteredTasks.length} tasks to process (removed ${doneTasks.length} done tasks)`);
    ...
    if (tasksForMonitoring.length > 0) {
      ...
      console.log('ℹ️ No pending tasks - skipping monitoring (already monitored)');
    }
    ...
    const alreadyDone = statusesBefore.filter(t => t.status === 'done' || t.status === 'processed_ga4');
    if (alreadyDone.length > 0) {
      console.error(`❌ CRITICAL: ${alreadyDone.length} tasks are already "done" before status update!`);
      console.error(`❌ Task IDs:`, alreadyDone.map(t => t.taskId).join(', '));
      console.error(`❌ This should not happen! These tasks should have been filtered out earlier.`);
    ...
    console.log(`🔄 Changing status to "done" for ${tasks.length} tasks BEFORE writing to sheets...`);
    const doneCount = await updateTaskStatus(tasks, 'done');
    console.log(`✅ Updated ${doneCount} tasks to "done" status`);

    if (doneCount !== tasks.length) {
      console.error(`❌ CRITICAL ERROR: Only ${doneCount}/${tasks.length} tasks were updated to "done"!`);
      console.error(`❌ ${tasks.length - doneCount} tasks were NOT updated and will be processed again!`);
    ...
    // KROK 7: Przygotuj i zapisz Google Ads Events (imitacja)
    const googleAdsEvents = tasks
      .map(task => prepareGoogleAdsEvent(task, pricingConfig))
      .filter(event => event !== null);
...
console.log('📤 Request body (with server-side filter):', JSON.stringify(requestBody));
...
allItems = data?.data?.items || [];
      ...
      console.log(`⚠️ Server returned ${allItems.length} tasks (filter may be ignored - will check statuses locally)`);
...
console.log('⚠️ Server-side filter not supported, falling back to client-side filtering');
    ...
    if (!fallbackResponse.ok) {
      console.error('❌ Stape API error:', fallbackResponse.status, await fallbackResponse.text());
...
console.log(`🔍 Filtering ${allItems.length} tasks locally (checking status)...`);
...
const taskId = item.key || item.id || 'unknown';
    ...
    if (isDone || isProcessed) {
      console.log(`  ⏭️ Skipping task ${taskId}: status="${status}" (already processed)`);
      return false;
    }
    ...
    console.log(`  ⚠️ Unknown status for task ${taskId}: "${status}" - skipping`);
    return false;
...
console.log(`✅ Tasks to process (pending OR monitored): ${tasksToProcess.length} (filtered from ${allItems.length} total)`);
...
console.warn(`⚠️ Collection at limit (${allItems.length}/100), but only ${tasksToProcess.length} tasks to process. ${doneCount} tasks are "done" and should be deleted.`);
}

// Jeśli wszystkie zadania są "done", zwróć pustą listę
if (tasksToProcess.length === 0 && allItems.length > 0) {
console.log(`ℹ️ All ${allItems.length} tasks are already processed (status: done/processed_ga4). Nothing to do.`);
}
...
values: rows
    ...
    console.log(`✅ Appended ${response.data.updates.updatedRows} rows to monitoring sheet`);
...
console.log(`🗑️ Found ${doneTasks.length} "done" tasks to delete (out of ${allTasks.length} total)`);
...
const documentId = task.key || task.id;
    ...
    try {
      ...
      if (deleteResponse.ok) {
        ...
        console.warn(`⚠️ Failed to delete task ${documentId}: ${deleteResponse.status} - ${errorText}`);
        failedCount++;
      }
    } catch (error) {
...
console.log(`✅ Deleted ${deletedCount} "done" tasks (${failedCount} failed)`);
return deletedCount;
}
...
const documentId = task.key || task.id;
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`⚠️ Failed to update task ${documentId}: ${response.status} - ${errorText}`);
        failedCount++;
      }
    } catch (error) {
...
console.log(`✅ Updated ${updatedCount}/${tasks.length} tasks to "${newStatus}" (${failedCount} failed)`);
...
// ============================================
async function getPricingConfigFromSheets() {
if (!SERVICE_ACCOUNT_KEY.client_email) {
...
range: 'Pricing_Config!A2:D100' // Wiersze 2-100 (pomijamy nagłówki)
    ...
    console.log(`📊 Loaded ${Object.keys(pricingMap).length} pricing keys from Sheets`);
    return pricingMap;
...
keyConfig = pricingConfig[withoutPrefix];
    ...
    if (keyConfig) {
      console.log(`🔧 Found pricing for "${withoutPrefix}" (without prefix from "${pricingKey}")`);
    }
...
console.log(`💰 getPricingValue: "${pricingKey}" → ${value} (platform: ${platform})`);
...
console.log(`🔍 Preparing GA4 event for: ${eventName}, task key: ${task.key}`);
...
console.log(`⏭️ SKIP GA4: consent_analytics_storage = denied for ${eventName}`);
...
console.log(`⏭️ SKIP GA4 (MP + arkusz GA4_Events): generate_lead — źródło GA4 = Web GTM`);
return null;
}
...
let pricingKey = taskData.biz_pricing_key;
    ...
    if (pricingKey) {
      ...
      const hasPrefix = pricingKey.startsWith('lead_') ||
      ...
      if (!hasPrefix) {
        ...
        console.log(`🔧 Added prefix: "${pricingKey}" (was: "${taskData.biz_pricing_key}")`);
      } else {
      ...
      console.log(`🔧 Built pricing_key: "${pricingKey}" from event="${eventName}" + product="${taskData.biz_product}"`);
    }
    ...
    if (pricingKey && pricingConfig) {
      ...
      console.warn(`⚠️ No pricing_key (${pricingKey}) or config for event ${eventName}`);
    }
...
const timestampMicros = new Date(taskData.time_occurred_iso_utc || new Date().toISOString()).getTime() * 1000;
...
// Prepare rows - UPEWNIJ SIĘ że wszystkie wartości są poprawnie sformatowane
const rows = validEvents.map((event, index) => {
// Konwertuj value na liczbę lub pusty string
let valueStr = '';
...
valueStr = numValue.toString();
    ...
    const row = [
      ...
      event.client_id || '',
      event.user_id || '',
      valueStr,  // Zawsze string (liczba lub pusty)
      event.currency || 'PLN',
      event.event_params || '{}',
      event.timestamp_micros || '' // ✅ NOWA KOLUMNA: Unix timestamp w mikrosekundach
    ];

    // Debug: pierwsze 3 wiersze
    if (index < 3) {
      console.log(`📤 Row ${index}:`, JSON.stringify(row));
      console.log(`📤 Value: ${event.value} (type: ${typeof event.value}) → "${valueStr}"`);
    }
...
console.log(`📤 Preparing to append ${rows.length} rows to GA4_Events`);
console.log(`📤 Using valueInputOption: RAW`); // Debug

// Append to GA4_Events sheet - UŻYJ RAW!
try {
const response = await sheets.spreadsheets.values.append({
spreadsheetId: GOOGLE_SHEET_ID,
range: 'GA4_Events!A:H', // ✅ ZMIENIONO: A:G → A:H (dodano timestamp_micros)
valueInputOption: 'RAW', // ← WAŻNE: RAW zamiast USER_ENTERED!
requestBody: {
values: rows
}
});

    console.log(`✅ Appended ${response.data.updates.updatedRows} GA4 events to Sheets`);
    return response.data.updates.updatedRows || 0;

} catch (error) {
console.error('❌ Error appending to GA4_Events:', error.message);
if (error.response && error.response.data) {
console.error('❌ Error details:', JSON.stringify(error.response.data, null, 2));
}
// Jeśli błąd, spróbuj wyczyścić arkusz lub użyj innej karty
throw error;
}
}
...
console.log(`⏭️ SKIP GA4 MP dla ${ga4Key}: brak GA4_${ga4Key.toUpperCase()}_MEASUREMENT_ID lub API_SECRET`);
continue;
}
...
if (!clientId) {
console.warn(`⚠️ GA4 MP: pomijam event ${event.event_name} - brak client_id i user_id`);
continue;
}
...
console.warn(`⚠️ GA4 MP [${ga4Key}]: ${event.event_name} (user_id=${String(event.user_id).slice(0, 12)}...) — brak ga_client_id, fallback client_id=user_id (id_oid). Lejek w GA4 może się NIE skleić z generate_lead z Web GTM. Sprawdź: Sortownia → identity_map → task_queue.ga_client_id.`);
}
...
if (event.timestamp_micros) params.timestamp_micros = event.timestamp_micros;
...
if (msgs.length > 0) {
console.error(`⚠️ GA4 MP [${ga4Key}] validation_messages dla ${mpEventName}:`, JSON.stringify(msgs, null, 2));
} else {
console.log(`✅ GA4 MP [${ga4Key}] DEBUG VALIDATE: payload OK dla ${mpEventName} (nie wysyłamy – tylko walidacja)`);
}
} catch (_) {
console.log(`✅ GA4 MP [${ga4Key}] DEBUG VALIDATE: odpowiedź dla ${mpEventName}`);
}
} else {
sent++;
console.log(`✅ GA4 MP [${ga4Key}]: wysłano ${mpEventName} (client_id: ${clientId.slice(0, 12)}...)${GA4_DEBUG_MODE ? ' [DEBUG_VIEW]' : ''}`);
}
} else {
console.error(`❌ GA4 MP: błąd ${res.status} dla ${mpEventName}`, responseText);
}
} catch (err) {
console.error(`❌ GA4 MP: fetch error dla ${event.event_name}:`, err.message);
}
}
...
const isMetaRelevant = owner === 'platform:meta_ads' || assist === 'platform:meta_ads';
if (!isMetaRelevant) {
console.log(`⏭️ SKIP Meta_Events: owner="${owner}", assist="${assist}" (not platform:meta_ads) for ${eventName}`);
return null;
}

// ✅ DEBUG: Sprawdź ctx_ip_address w taskData
console.log(`🔍 DEBUG Meta: taskData.ctx_ip_address =`, taskData.ctx_ip_address);
console.log(`🔍 DEBUG Meta: taskData.ctx_user_agent =`, taskData.ctx_user_agent);
...
const eventTime = Math.floor(new Date(taskData.time_occurred_iso_utc || new Date().toISOString()).getTime() / 1000);
...
console.warn(`⚠️ Missing ctx_user_agent for Meta CAPI event ${metaEventName} - event quality may be reduced`);
}
...
console.log(`✅ Meta: Added client_ip_address = ${taskData.ctx_ip_address}`);
} else {
console.warn(`⚠️ Meta: Missing ctx_ip_address in taskData for event ${metaEventName}`);
}
...
valueStr = numValue.toString();
    ...
    return [
      ...
      event.currency || 'PLN',
      event.user_data || '{}',
      event.client_user_agent || '', // ✅ NOWA KOLUMNA: User Agent
      event.client_ip_address || '', // ✅ NOWA KOLUMNA: IP Address
      event.event_time || '' // ✅ NOWA KOLUMNA: Unix timestamp (sekundy)
    ];
...
const response = await sheets.spreadsheets.values.append({
spreadsheetId: GOOGLE_SHEET_ID,
range: 'Meta_Events!A:K', // ✅ ZMIENIONO: A:H → A:K (dodano 3 kolumny)
...
values: rows
    ...
    console.log(`✅ Appended ${response.data.updates.updatedRows} Meta events to Sheets`);
    return response.data.updates.updatedRows || 0;

} catch (error) {
console.error('❌ Error appending to Meta_Events:', error.message);
throw error;
}
}
...
const ok = typeof resourceName === 'string' &&
resourceName.startsWith('customers/') &&
resourceName.includes('/conversionActions/');

if (!ok) {
throw new Error(`Bad conversionActionResourceName (expected "customers/{cid}/conversionActions/{id}"): ${resourceName}`);
}
}
...
} else if (job.hash_email_sha256 && isValidSha256HexLower(job.hash_email_sha256)) {
userIdentifiers.push({
hashedEmail: job.hash_email_sha256,
userIdentifierSource: 'FIRST_PARTY'
});
noteParts.push('Email hash used from contract (raw email missing/invalid).');
} else if (job.biz_email) {
noteParts.push('Invalid email format after normalization; sending without email EC.');
}
...
if (job.biz_phone.indexOf('+') === 0 && /^\+?\d{10,15}$/.test(job.biz_phone.replace('+', ''))) {
...
} else if (job.hash_phone_sha256 && isValidSha256HexLower(job.hash_phone_sha256)) {
userIdentifiers.push({
hashedPhoneNumber: job.hash_phone_sha256,
userIdentifierSource: 'FIRST_PARTY'
});
noteParts.push('Phone hash used from contract (raw phone missing/invalid).');
} else if (job.biz_phone) {
noteParts.push('Invalid phone format after normalization; sending without phone EC.');
}

if (userIdentifiers.length === 0) {
return { note: noteParts.join(' ') || 'No valid PII for Enhanced Conversions; sending without EC.' };
}
...
if (userIdentifiers && userIdentifiers.length) payload.userIdentifiers = userIdentifiers;

if (!clickField) {
const platformNone = owner === 'platform:none' || assist === 'platform:none';
if (platformNone && userIdentifiers && userIdentifiers.length) {
return {
status: 'success_ec_only',
note: (note ? note + ' ' : '') + 'platform:none — upload bez gclid (Enhanced Conversions for Leads).',
payload
};
}
return {
...
return {
status: userIdentifiers && userIdentifiers.length ? 'success' : 'success_no_ec',
note,
payload
};
}
...
if (!job.order_id || job.order_id.trim() === '') {
return { status: 'failed_final', note: 'Missing order_id for RESTATE (must come from Akt Własności).' };
}
...
const d = daysBetween(job.base_time_occurred_iso_utc, job.time_occurred_iso_utc);
...
const currency = (job.biz_currency || cfg.currencyCode || 'PLN').toUpperCase();
...
const baseMissing = new Set(['NO_BASE_CONVERSION_YET', 'CONVERSION_NOT_FOUND']);
if (baseMissing.has(errCode)) {
const waits = cfg.baseMissingRetryMinutes;
if (attemptIndex >= waits.length) {
return { status: 'failed_final', note: 'Base conversion never arrived after max retries (race condition).' };
}
return {
...
const transient = httpStatus === 429 ||
(httpStatus !== null && httpStatus >= 500) ||
['INTERNAL_ERROR', 'DEADLINE_EXCEEDED', 'RESOURCE_EXHAUSTED'].includes(errCode);

if (transient) {
const waits = cfg.transientRetryMinutes;
if (attemptIndex >= waits.length) {
return { status: 'failed_final', note: `Transient error exhausted retries: ${errCode}` };
}
return {
status: 'retry',
...
// reszta = permanent
return { status: 'failed_final', note: `Google Ads API error (permanent): ${errCode}` };
}
...
const owner = taskData.owner || '';
const assist = taskData.assist || '';
const isGoogleAdsRelevant = owner === 'platform:google_ads' || assist === 'platform:google_ads' ||
  owner === 'platform:none' || assist === 'platform:none';
if (!isGoogleAdsRelevant) {
console.log(`⏭️ SKIP GoogleAds_Events: owner="${owner}", assist="${assist}" for ${eventName}`);
return null;
}
...
const consentAd = taskData.consent_ad_storage;
if (consentAd === 'denied') {
console.log(`⏭️ SKIP Google Ads: consent_ad_storage = denied for ${eventName}`);
return null; // Funkcja już zwraca null dla skipped events
}
...
const conversionDateTime = new Date().toISOString().replace('T', ' ').substring(0, 19);
...
valueStr = numValue.toString();
    ...
    return [
      ...
      event.user_data || '{}'  // Dodane: user_data (email, phone, external_id)
    ];
...
const response = await sheets.spreadsheets.values.append({
spreadsheetId: GOOGLE_SHEET_ID,
range: 'GoogleAds_Events!A:H', // Zmienione z A:G na A:H (dodana kolumna user_data)
valueInputOption: 'RAW',
requestBody: {
values: rows
}
    ...
    console.log(`✅ Appended ${response.data.updates.updatedRows} Google Ads events to Sheets`);
    return response.data.updates.updatedRows || 0;
...
let pricingKey = taskData.biz_pricing_key;
if (pricingKey) {
const hasPrefix = pricingKey.startsWith('lead_') || pricingKey.startsWith('sql_') || pricingKey.startsWith('rejected_');
...
return owner === 'platform:google_ads' || assist === 'platform:google_ads' ||
  owner === 'platform:none' || assist === 'platform:none';
}
 ...
 */
function tryPushGoogleAdsClickConversion(conversions, conv, taskData, eventName, owner, assist) {
...
if (userIdentifiers && userIdentifiers.length) conv.userIdentifiers = userIdentifiers;
...
if (isPlatformNoneOwner(owner, assist) && userIdentifiers && userIdentifiers.length) {
console.log(
...
console.warn(`⚠️ Google Ads: pomijam ${eventName} — brak attr_gclid / attr_gbraid / attr_wbraid`);
}
return false;
}
...
const reqId = (typeof res.headers.get === 'function' && (res.headers.get('request-id') || res.headers.get('x-goog-request-id'))) || '';
if (reqId) console.log(`📎 Google Ads ${operationLabel} request-id: ${reqId}`);
if (data.jobId != null && data.jobId !== '') {
console.log(`📎 Google Ads ${operationLabel} jobId: ${String(data.jobId)}`);
}
const results = data.results || [];
if (results.length > 0) {
console.log(`📎 Google Ads ${operationLabel} results (${results.length}):`, JSON.stringify(results));
} else {
console.warn(`⚠️ Google Ads ${operationLabel}: pusty results[] (HTTP OK) — sprawdź partialFailureError poniżej`);
}
if (data.partialFailureError) {
console.error(`⚠️ Google Ads ${operationLabel} partialFailureError:`, JSON.stringify(data.partialFailureError));
}
}
...
const timeOccurred = taskData.time_occurred_iso_utc || new Date().toISOString();
const conversionDateTime = timeOccurred.slice(0, 19).replace('T', ' ') + '+00:00';

if (eventName === 'generate_lead') {
if (!orderId) { console.warn('⚠️ Google Ads: pomijam generate_lead bez order_id'); continue; }
const { value, currency } = getGoogleAdsValueForTask(taskData, eventName, pricingConfig);
const conversionValue = (value != null && !isNaN(value)) ? Number(value) : 0.01;
const conv = {
...
console.log(`📡 Google Ads API REST v${GOOGLE_ADS_API_VERSION} (customer ${customerId})`);
...
console.log(`✅ Google Ads API: uploadClickConversions HTTP OK, results.length=${results.length}`);
} else {
console.error('❌ Google Ads API uploadClickConversions:', res.status, text);
}
} catch (e) {
...
console.log(`✅ Google Ads API: uploadConversionAdjustments HTTP OK, results.length=${adjResults.length}`);
} else {
console.error('❌ Google Ads API uploadConversionAdjustments:', res.status, text);
}
} catch (e) {
console.error('❌ Google Ads API uploadConversionAdjustments:', e.message);
}
}

...
function normalizeTasksEventNames(tasks) {
  tasks.forEach((task) => {
    ...
    if (task.data) {
      task.data.biz_value = normalizeBizValueForTask(task.data.biz_value);
    }
  });
}
...
function enrichPurchaseBizValues(tasks, pricingConfigProd, pricingConfigSandbox) {
  tasks.forEach((task) => {
    ...
    const pricing =
      sheetId === GOOGLE_SHEET_ID_SANDBOX
        ? pricingConfigSandbox
        : pricingConfigProd;
    const value = resolvePurchaseValueFromPricing(
      taskData,
      pricing,
      "google_ads",
    );
    if (value != null) {
      taskData.biz_value = value;
    }
  });
}
...
functions.http("processTaskQueue", async (req, res) => {
  ...
  try {
    ...
    enrichPurchaseBizValues(tasks, pricingConfigProd, pricingConfigSandbox);
    console.log(
    ...
    console.log(
      `✅ Sent ${metaCapiSent} Meta events to Conversions API` +
        (sandboxInstaFormAllowed > 0
          ? ` (incl. ${sandboxInstaFormAllowed} sandbox Instant Form)`
          : " (prod tasks)"),
    );
...
async function appendToGoogleSheetsForId(tasks, spreadsheetId) {
  ...
  const rows = tasks.map((task) => {
    ...
    return [
      ...
      normalizeBizValueForTask(taskData.biz_value), // (J) - ISTNIEJĄCE
...
function prepareGA4Event(task, pricingConfig) {
  ...
  if (eventName === "purchase") {
    value =
      taskData.biz_value != null && taskData.biz_value !== ""
        ? parseFloat(taskData.biz_value)
        : null;
    if (value == null || Number.isNaN(value)) {
      value = resolvePurchaseValueFromPricing(
        taskData,
        pricingConfig,
        "ga4",
      );
      console.log(`💰 purchase value: ${value} (pricing fallback)`);
    } else {
  ...
  if (eventName === "purchase") {
    const direct =
      taskData.biz_value != null && taskData.biz_value !== ""
        ? parseFloat(taskData.biz_value)
        : null;
    value = direct != null && !Number.isNaN(direct) ? direct : null;
    if (value == null && pricingConfig) {
      value = resolvePurchaseValueFromPricing(
        taskData,
        pricingConfig,
        "google_ads",

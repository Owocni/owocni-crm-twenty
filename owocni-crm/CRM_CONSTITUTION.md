...
| INV-7 | **Tożsamość fail-closed.** Stape Store niedostępny podczas resolve → NIE mintuj id_oid, kolejkuj. **NIGDY auto-merge** — propozycje merge tylko po corporate `company_domain_key` (§5.8.2); free-mail wyłączone. Dwa paid id_oid → T5 ręcznie. Mechanika → IDENTITY. | Fail-open = duplikat id_oid = rozdwojenie tożsamości (nieodwracalne). | Jeden klient z dwoma profilami, zepsuta atrybucja. | Właściciel + ADR | D:CORE |

...
|---|---|---|---|---|---|
| 2026-09-15 | **createLead: lista zablokowanych emaili** | `artur@maxseo.pl` + dopisywanie w `shared/leadEmailBlocklist.js` (albo env `LEAD_EMAIL_BLOCKLIST`). Skip Person/Opp na formularzu, Meta i `leads@`. Twenty Blocklist zostaje na Email Sync. | **TAK** | Composer | Live `twenty-crm-worker-sandbox` rev `00076-dob`, build `2026-09-15-lead-email-blocklist`. |
...
| 2026-09-11 | **Owocni Mail deploy 0.1.123** | Wysyłka bierze żywą treść z edytora (ACK); brak odczytu = stop, nie suchy szablon / pusta odpowiedź. | **TAK** | Composer | Live `version=0.1.123` checksum `c458c84b4c49eacb1318d74f00fdd64c`. **Wycofane** — iframe w hoście nie oddaje treści, Gosia nie mogła wysłać. |
| 2026-09-11 | **Owocni Mail deploy 0.1.125** | Szkice z edytora idą na API (nie SPA); odliczanie 15 s wysyła pełne HTML bez limitu keepalive 64 KB; meta szkicu nie nadpisuje pustym HTML. | **TAK** | Composer | Live `version=0.1.125` checksum `39235e329c2f0b4d6e56bf877621824c`. **Wycofane** — nadal suchy szablon / sama stopka. |
| 2026-09-11 | **Owocni Mail deploy 0.1.127** | Token bez przebudowy iframe (`MAIL_SET_AUTH`); wysyłka jak 0.1.126. | **TAK** | Composer | **Wycofane** — edycje z edytora znowu nie szły (iframe bez tokenu w srcDoc). Checksum `332b47b92481dfeac351d183a403c777`. |
| 2026-09-11 | **Owocni Mail deploy 0.1.128** | Cofnięcie 0.1.127: z powrotem remount iframe z tokenem (zachowanie 0.1.126). | **TAK** | Composer | Live `version=0.1.128` checksum `c6ba445b16b45ed0b56dfbf32adc0d26`. Hard refresh. Wysyłka jak rano; wraca przebudowa edytora przy tokenie. |
...
| 2026-09-12 | **Queue triage T1 (fale 1–2)** | A/B zdjęte z kolejki Gosi; C → Maciej COPYWRITING; 80× NEW+followUp Gosi/Marty (`isFollowUp=false`). Etapu/SQL/WON nietknięte. | **TAK** | Composer | Runbook `QUEUE_TRIAGE_20260912.md`. Manifest `exports/queue_cleanup/20260912T052202Z/`. |
| 2026-09-12 | **Queue triage fala 3a** | 14 holding Sortownia (sample-week surplus) → Marta 11 / Gosia 3. PD/PAYING na Owocni bez zmian. | **TAK** | Composer | `wave3_holding_sortownia_apply.json`. |
| 2026-09-12 | **Queue triage T1 czat** (testy, Batko, Inkflow, dump vs BB) | DELETE **8** Opp (7 testów + Fakturownia DEMO) + 7 orphan Person. KEEP `a88aa583-646d-41f2-944f-20c988f63e3f`. Batko: BB miss / PD miss (bez kasowania). Inkflow `00fad7e0-…` → merge_leads LOSER vs WON Roberta `3fcd51f3-…` (PD 6819, 4900 PLN). Dump 26 VI–3 VII: 28 kart, BB **20 hit / 8 miss** — nie rozdane, nie auto-LOST. Open Marta/Gosia/Maciej **572**; bitrixDealId 344; Sortownia 390; piątek 78 patched + 0 creates (33 dziury). Karty=tak, maile OUT BB=nie. | **TAK** | Composer | Manifest `exports/queue_cleanup/20260912T052202Z/t1_tests_batko_inkflow.json`. |
| 2026-09-12 | **T1 GO-2: Batko hash + dump=BB owner + C-Δ IMAP** | Batko `34b12db0-…` → **Gosia** (`idOid` sum 1740 parzysty). Dump BB-hit: **17** owner z BB (Marta 14 / Maciej 2 / Gosia 1); etap bez QUALIFIED/WON; `paulinaslopiecka` WON w BB zostaje NEW. Skip uid 293/294 (invitely, maksiucabajk) + 8 miss. IMAP: 2× skitomasz + 3× Marta C-Δ od 15:30Z; dump poza skitomasz = 0 Sent w BB. | **TAK** | Composer | `t1_tests_batko_inkflow.json` + `exports/bb_sync/cdelta_t1_20260912/`. |
| 2026-09-12 | **T1 Paulina WON no_emit + token-spam DELETE** | `c9f1add0-…` `paulinaslopiecka@` → stage WON + `srcSystem=BETTER_BITRIX_LEGACY` (inbound `SKIP_LEGACY_IMPORT`, bez `purchase` na Ads). Token-spam Sortownia: **7** Opp + Person usunięte (sitko jak createLead 8 IX); ikuzi Person bez karty też. | **TAK** | Composer | `t1_tests_batko_inkflow.json` block6. |
| 2026-09-12 | **T1 BB↔Twenty etap + C-Δ LEADS-parent** | Matcher + holding: 230 BB (30d, closed). **22 PATCH** HTTP 200 (13 NEW→CONTACTED w tym `kontaktjjcestates` `bb:9318`; 5 CONTACTED→PROPOSAL; 2 PROPOSAL→LOST; 1 PROPOSAL→CONTACTED; 1 link ID). QUALIFIED nietknięte. **0 create**. C-Δ `--allow-other-channel` od 5 IX: Gosia 10 + Marta 11 + copy 3 APPEND do `BB Archive` (w tym OUT Jakuba 7 IX). | **TAK** | Composer | `exports/bb_sync/runs/20260912T072500Z/` + `cdelta_t1_20260912/apply_*073*.json`. |
| 2026-09-12 | **T1 flaga Do odpisania = odwrotność BB follow-up** | BB `is_follow_up=true` (czekamy) ≠ Twenty `isFollowUp` (nasza tura). **58 PATCH**: 7 zdjęte (były w kolejce mimo BB waiting) + 12 Sortownia bez BB 90d zdjęte + 39 włączone (BB nasza tura). Po: Marta **19=19**, Gosia **0=0**, Maciej 31 vs 55 (24 BB bez karty Twenty, głównie stare naming). Zero BB-waiting w „Do odpisania”. | **TAK** | Composer | `runs/20260912T130000Z/followup_flag_apply.json`. |
| 2026-09-12 | **T1 flaga: live view, nie MV** | Wieczorny sync czytał `leads_extended_materialized` (nie ma świeżych formularzy). Gosia BB **9389** `malaga82@op.pl` i Marta **9388** `afis@onet.pl` (dziś, `fu=false`) zeszły z kolejki jako `sortownia_no_bb90`. **3 PATCH** `isFollowUp=true` + link `bb:` (9389/9388) + Marta **9332** `e.arcaba` (live `fu=false`). `fetch_bb_leads` → `leads_extended_view`. | **TAK** | Composer | `runs/20260912T182500Z/followup_flag_live_fix.json`. |
| 2026-09-12 | **T1 gdyniasy: skleić OUT Marty** | BB 9387: Marta wysłała 09:25, `fu=true`. C-Δ APPEND wylądował w osobnym wątku (`direction` puste). PATCH Message → wątek rodzica IMAP + `OUTGOING`/`MARTA`; Opportunity `isFollowUp=false`, `lastContactAt` = wysyłka. Etap CONTACTED (BB negotiations) nietknięty. | **TAK** | Composer | `runs/20260912T204000Z/gdyniasy_stitch.json`. |
| 2026-09-12 | **T1 poniedziałek: flaga live + sitko create** | Align `isFollowUp` z live BB na 470 spiętych kartach: **5 OFF** (Marta, BB czekamy). 8 skip (przyszłe snooze). Sitko BB bez karty: 99 wierszy → po weryfikacji **96 prawdziwych dziur**, wszystkie Maciej, **0 z ostatnich 30 dni** (2 fałszywe: WON + `maciej@owocni.pl`). **0 create.** | **TAK** | Composer | `runs/20260912T205000Z/`. |
| 2026-09-12 | **Owocni Mail deploy 0.1.130** | Filtr pustej treści nie zjada szablonów z `<blockquote>` ani tekstu wpisanego w stopkę. Kursor przed stopką. | **TAK** | Composer | Live `version=0.1.130` checksum `12ef73e14bcdaa402244deaf51234998`. **Nie naprawiło wysyłki** — screenshot 07:57: widać tekst, toast „treść pusta”. |
| 2026-09-12 | **Owocni Mail deploy 0.1.131** | Edytor wizualny bez zagnieżdżonego `srcDoc`. `flushHtml` czyta `contenteditable` w React. | **TAK** | Composer | **Wycofane** — edytor martwy, ucięta stopka. Checksum `fb6d1197a8a77ba06d024de351c773ce`. |
| 2026-09-12 | **Owocni Mail deploy 0.1.132** | Cofnięcie 0.1.131 → iframe jak 0.1.130. Plan Mariusza osobno. | **TAK** | Composer | Live `version=0.1.132` checksum `3982aecfabaddf6e6ede85d9c58b08ff`. Nadal toast „sama stopka” (filtr sobotni). |
| 2026-09-12 | **Owocni Mail deploy 0.1.133** | Rollback wysyłki do 0.1.128 / kod ~10:17 (token w srcDoc, szkic, bez filtra stopki). | **TAK** | Composer | Live `version=0.1.133` checksum `9610cefb7e86b9b9f57a04841794a723`. Hard refresh. Test: wpisać, ~2 s, Wyślij na skrzynkę kontrolną. |
...
| 2026-09-11 | Twenty z powrotem prod Ads; BB `/lead` sandbox; sample-week OFF | Sprzedaż wraca do Twenty; Google znowu z kart Twenty | `decision` | `ROLLFORWARD_20260911.md`. Furtka Meta OFF. |
| 2026-09-11 | Gosia: oferta / odpowiedź = suchy szablon albo sama stopka | Edycje z edytora nie szły przy Wyślij; 0.1.122–125 i 0.1.127 nie pomogły | `incident` | Rollback **0.1.128** = zachowanie 0.1.126 / kod z 10:17 (0.1.121). |
| 2026-09-12 | Composer: widać wpisany tekst, toast „Treść maila jest pusta” | Nested `srcDoc` w sandboxie FC: typing działa, `postMessage`/flush do React nie; payload = sama stopka | `incident` | Deploy **0.1.131** — `contenteditable` w drzewie React, bez zagnieżdżonego iframe. |

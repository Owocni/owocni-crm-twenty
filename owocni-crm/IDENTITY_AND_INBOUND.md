...
**Formularz paid:** `Formularz → Sortownia (oid_init → generate_lead) → id_oid + verified + vbb_eligible → adapter crm:twenty_create_lead → Twenty`. Sortownia Etap A: **nie przepisujemy** logiki Owner / Akt / 90 dni. **Sitko token-spam (2026-09-08):** worker `createLead` nie tworzy Person/Opp, gdy cała `biz_message` to jeden losowy token `[A-Za-z0-9]{12,}` z ≥3 wielkimi i ≥3 małymi literami (np. `VxIvkhPmufoiYZhTVdds`). Mail `leads@` / `TWENTY_EMAIL` — **bez** tego sitka. **Sitko zablokowanych emaili (2026-09-15):** `createLead` (formularz, Meta, `leads@`) nie mintuje Person/Opp, gdy email jest na liście `integrations/cloud-functions/twenty-crm-worker/shared/leadEmailBlocklist.js` (dokładny adres albo `@domena.pl`). Dopisz tam kolejną pozycję i zdeployuj worker; na szybko: env `LEAD_EMAIL_BLOCKLIST`. To **nie** zastępuje Twenty Blocklist (Email Sync) — tam nadal wrzucamy ten sam handle, żeby poczta nie wchodziła.
